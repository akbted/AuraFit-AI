...
import { RefreshCw, CheckCircle2, AlertCircle, Play, Square, Timer, Mic, MicOff } from 'lucide-react';
...
const WorkoutSessionContent = ({ activeExercise, onSwitchExercise, onReset, inputVideoRef }) => {
  ...
  const handleVoiceCommand = useCallback((action, rawCommand) => {
    console.log('Voice command received:', action, rawCommand);
    ...
    switch (action) {
      ...
      case 'crunches':
      case 'legRaises':
        handleSwitchWithEnd(action);
        break;
      default:
        console.log('Unknown command:', rawCommand);
    }
  }, [isSessionActive, startSession, endSession, completeSet]);

  const { isListening, isSupported, transcript, toggleListening } = useVoiceCommands(handleVoiceCommand);
      ...
      */}
      ...
      <div className="flex-1 bg-black rounded-3xl overflow-hidden relative shadow-2xl border border-slate-800">
        ...
        <div className="absolute top-6 left-6 z-20 flex flex-wrap gap-2">
          <div className={`px-4 py-2 rounded-full backdrop-blur-md flex items-center gap-2 text-sm font-medium ${connected
          ...
          {isSupported && (
            <button
              ...
              className={`px-4 py-2 rounded-full backdrop-blur-md flex items-center gap-2 text-sm font-medium transition-all ${isListening
                  ? 'bg-red-500/20 text-red-300 border border-red-500/30 animate-pulse'
                  : 'bg-slate-700/20 text-slate-300 border border-slate-600/30 hover:bg-slate-600/30'
                }`}
            >
        ...
        {transcript && (
          <div className="absolute top-24 left-6 right-6 z-20">
            <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-4 max-w-md">
      ...
      <div className="lg:w-96 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl flex-1 flex flex-col justify-center items-center text-center relative overflow-hidden group">
          <div className={`absolute inset-0 opacity-5 transition-colors duration-300 ${exerciseType === 'timed'
          ...
          <div className={`mt-6 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border transition-all ${state === 'DOWN' ? 'bg-blue-100 text-blue-700 border-blue-200' :
        ...
        <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl">
          ...
          <button
            onClick={completeSet}
            disabled={!connected}
            className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 mb-3 transition-all ${connected
